...
class AsyncNoveltyJudge:
    ...
    async def assess_novelty_with_rejection_sampling_async(
        ...
        try:
            # Compute similarities with programs in island            if parent_program.island_idx is None:
            ...
            # First check: embedding similarity threshold            if max_similarity <= self.sync_judge.similarity_threshold:
                logger.info(
            ...
            # High similarity detected - check with LLM if configured            should_reject = True
            novelty_cost = 0.0
    ...
    async def _check_llm_novelty_async(
        ...
        # Import novelty prompts        from ..prompts import NOVELTY_SYSTEM_MSG, NOVELTY_USER_MSG
        ...
        try:
            ...
            # Parse the response            is_novel = content.upper().startswith(
                "NOVEL"
