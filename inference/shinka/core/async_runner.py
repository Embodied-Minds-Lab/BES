...
class ShinkaEvolveRunner:
    ...
    def __init__(
        ...
        backward_callback: Optional[Any] = None,
    ):
        ...
        # FIFO queue of patch_type overrides for upcoming forward proposals.
        # Backward callbacks (e.g. expand-exhausted rebuild) push entries here so
        # the next forward proposal forces a particular patch_type instead of the
        # usual random sample. The sampler call site below pops one entry per
        # proposal when non-empty.
        self.queued_patch_types: List[str] = []
        ...
        # Backward-search gate.
        # `backward_callback` is an async callable invoked after each completed-job batch
        # (signature: `await backward_callback(jobs, runner)`). While it runs, `backward_pause`
        # is cleared so the proposal coordinator stops dispatching new forward proposals.
        # Set `backward_callback=None` to disable backward entirely.
        self.backward_callback = backward_callback
        self.backward_pause = asyncio.Event()
        self.backward_pause.set()  # forward enabled by default
    ...
    def _build_previous_attempts(self, k: int = 10) -> str:
        """Render top-k distinct previously-tried algorithm names from the DB.

        Used by `DIFF_ABLATE_ITER_MSG` (`{previous_attempts}` placeholder) so the
        pivot prompt sees a concrete ban list — what algorithm classes have
        already been explored and how they scored — instead of looping within
        the same family.

        Failures are logged and turned into a benign placeholder string; this
        helper must never raise into the forward proposal path.
        """
        try:
            db_path = getattr(getattr(self.db, "config", None), "db_path", None)
            if not db_path or not Path(db_path).exists():
                return "(no previous attempts recorded yet)"
            conn = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True, timeout=5)
            try:
                rows = conn.execute(
                    "SELECT json_extract(metadata,'$.patch_name') AS name, "
                    "json_extract(metadata,'$.patch_type') AS ptype, "
                    "MAX(combined_score) AS score, MIN(generation) AS gen "
                    "FROM programs "
                    "WHERE correct=1 AND name IS NOT NULL "
                    "GROUP BY name "
            ...
            if not rows:
                return "(no previous attempts recorded yet)"
            lines = [
                "## Previously attempted approaches (already explored — propose "
                "something STRUCTURALLY different from ALL of these, not a minor "
                "variant):"
            ]
            for name, ptype, score, gen in rows:
                score_str = f"{score:.4f}" if score is not None else "?"
                lines.append(f"- {name} ({ptype}, score {score_str}, gen {gen})")
            return "\n".join(lines)
        except Exception as e:
            logger.warning(f"_build_previous_attempts failed: {e}")
            return "(could not load previous attempts)"
    ...
    async def _update_prompt_fitness(
        ...
        improvement: float,
        correct: bool = True,
        program_public_metrics: Optional[Dict[str, Any]] = None,
        program_private_metrics: Optional[Dict[str, Any]] = None,
    ):
        """
        ...
        Args:
            ...
            program_public_metrics: Optional dict so percentile can be computed
                in adaptive effective-score space (raw + bw).
            program_private_metrics: Optional dict (carries reported_sum_of_radii).
        """
        if not prompt_id or not self.prompt_db:
            return

        try:
            # Compute the percentile rank of this program's score in the
            # adaptive bucket-interpolated effective-score space when raw/bw
            # are available, so prompt fitness tracks the same ordering as
            # parent/archive selection. Only correct programs are scored.
            if correct:
                priv = program_private_metrics or {}
                pub = program_public_metrics or {}
                raw_val = priv.get("reported_sum_of_radii")
                bw_val = pub.get("backward_score")
    ...
    async def _proposal_coordinator_task(self):
        ...
        while not self.should_stop.is_set():
            try:
                # Pause forward dispatch while backward (rebuild/expand/rescore) holds the gate.
                if not self.backward_pause.is_set():
                    if self.verbose:
                        logger.info("[backward_gate] forward paused; waiting for backward to finish")
                    await self.backward_pause.wait()
                    if self.verbose:
                        logger.info("[backward_gate] forward resumed")
    ...
    async def _generate_evolved_proposal(
        ...
        # Initialize novelty tracking variables        novelty_checks_performed = 0
        ...
        for attempt in range(self.evo_config.max_novelty_attempts):
            for resample in range(self.evo_config.max_patch_resamples):
                try:
                    ...
                    if not patch_result:
                        ...
                        logger.info(
                            f"[resample_redraw] gen={generation} "
                            f"novelty={attempt+1}/{self.evo_config.max_novelty_attempts} "
                            f"resample={resample+1}/{self.evo_config.max_patch_resamples} "
                            f"stage=proposal reason=no_result"
                        )
                        continue
                    ...
                    if not success:
                        ...
                        logger.info(
                            ...
                            f"novelty={attempt+1}/{self.evo_config.max_novelty_attempts} "
                            f"resample={resample+1}/{self.evo_config.max_patch_resamples} "
                            f"stage=proposal patch_type={meta_patch_data.get('patch_type')} "
                            f"reason={str(last_failure_reason)[:120]!r}"
                        )
                        continue
            ...
            if self.novelty_judge:
                ...
                if should_check:
                    ...
                    # Update costs and metadata from novelty assessment                    novelty_cost_from_check = novelty_metadata.get(
                        "novelty_total_cost", 0.0
                    )
                    novelty_total_cost += (
                    ...
                    logger.info(
                        ...
                        f"novelty={attempt+1}/{self.evo_config.max_novelty_attempts} "
                        f"resample={resample+1}/{self.evo_config.max_patch_resamples} "
                        f"stage=novelty patch_type={meta_patch_data.get('patch_type')} "
                        f"reason={str(last_failure_reason)[:120]!r}"
                    )
                else:
                    proposal_accepted = True
        ...
        # Add meta-recommendations/summary/scratchpad to meta_patch_data        if meta_recs is not None:
        ...
        if proposal_accepted:
            # Add novelty check information to meta_patch_data if any checks were performed            if generation > 0 and novelty_checks_performed > 0:
    ...
    async def _run_patch_async(
        ...
        try:
            ...
            # Pop a queued patch_type override (set by backward callback after
            # expand-exhausted rebuild, etc.). Each queued entry is consumed by
            # exactly one forward proposal.
            forced_patch_type = (
                self.queued_patch_types.pop(0)
                if self.queued_patch_types
                else None
            )
            if forced_patch_type and self.verbose:
                logger.info(
                    f"Popping queued patch_type override: {forced_patch_type} "
                    f"(remaining queue: {self.queued_patch_types})"
                )

            # Build "previous attempts" summary from the DB (top-K distinct
            # patch_names by best combined_score). Used by diff_ablate to tell the
            # LLM which algorithm classes have already been explored, so the
            # pivot proposal doesn't loop within the same family.
            prev_attempts = self._build_previous_attempts()
            ...
            patch_sys, patch_msg, patch_type = self.prompt_sampler.sample(
                ...
                forced_patch_type=forced_patch_type,
            ...
            for patch_attempt in range(self.evo_config.max_patch_attempts):
                ...
                if not response or not response.content:
                    error_str = "LLM response content was None."
                    logger.info(
                        f"[patch_attempt_failed] gen={generation} "
                        f"novelty={novelty_attempt} resample={resample_attempt} "
                        f"patch_attempt={patch_attempt+1} patch_type={patch_type} "
                        f"reason=empty_llm_response"
                    )
                ...
                if error_msg is None and num_applied > 0:
                    ...
                    logger.info(
                        ...
                        f"novelty={novelty_attempt} resample={resample_attempt} "
                        f"patch_attempt={patch_attempt+1} patch_type={patch_type} "
                        f"num_applied={num_applied} reason={error_str[:120]!r}"
                    )
            ...
            # All attempts failed
            logger.info(
                f"[patch_all_attempts_failed] gen={generation} "
                f"novelty={novelty_attempt} resample={resample_attempt} "
                f"patch_type={patch_type} max_patch_attempts={self.evo_config.max_patch_attempts} "
                f"last_error={(error_str if 'error_str' in locals() else 'unknown')[:120]!r}"
            )
            meta_patch_data = {
                "api_costs": total_costs,
                "patch_type": patch_type,
            ...
            if system_prompt_id and self.evo_config.evolve_prompts:
                ...
                async with prompt_lock:
                    ...
                    await self._update_prompt_fitness(
                        ...
                        program_public_metrics=program.public_metrics,
                        program_private_metrics=program.private_metrics,
                    )
    ...
    async def _process_completed_jobs_safely(
        ...
        # Backward-search hook: pause forward, run callback (rebuild / expand / rescore),
        # then resume. The callback is None if backward is disabled.
        if self.backward_callback is not None and successfully_processed:
            await self._run_backward_callback(successfully_processed)
    ...
    async def _run_backward_callback(
        self, successfully_processed: List[AsyncRunningJob]
    ):
        """Pause forward dispatch, invoke the user backward callback, then resume.

        The callback (set via `backward_callback` constructor arg) is awaited with
        `(successfully_processed_jobs, runner)`. While the callback runs, the proposal
        coordinator is gated on `backward_pause`, so no new forward proposals start.
        In-flight proposals are NOT cancelled — they will complete naturally and any
        eval already running will read whatever goal-tree state is on disk.
        """
        self.backward_pause.clear()
        try:
            if self.verbose:
                logger.info(
                    f"[backward_gate] running callback over "
                    f"{len(successfully_processed)} just-completed gen(s)"
                )
            await self.backward_callback(successfully_processed, self)
        except Exception as e:
            logger.error(f"[backward_gate] callback raised: {type(e).__name__}: {e}")
        finally:
            self.backward_pause.set()
            self.slot_available.set()
