...
from shinka.prompts import (
    ...
    DIFF_ABLATE_ITER_MSG,
    FULL_ITER_MSG,
    FULL_SYS_FORMATS,
    CROSS_SYS_FORMAT,
    CROSS_ITER_MSG,
    CROSS_COMBINE_ITER_MSG,
    CROSS_TRANSLOCATE_ITER_MSG,
    get_cross_component,
    get_cross_combine_component,
    get_cross_translocate_component,
    FIX_SYS_FORMAT,
    FIX_ITER_MSG,
...
class PromptSampler:
    ...
    def sample(
        ...
        # Caller can override patch_type selection (e.g. backward callback queues
        # "diff_ablate" after expand-exhausted rebuild to force a pivot). When
        # cross-variants are forced but no inspirations are available, fall back
        # to random sampling so we don't crash the cross context-builder branch.
        no_inspirations = (
            len(archive_inspirations) == 0 and len(top_k_inspirations) == 0
        )
        forced_ok = (
            forced_patch_type is not None
            and not (no_inspirations and forced_patch_type.startswith("cross"))
        )
        if forced_ok:
            patch_type = forced_patch_type
            logger.info(f"Forced patch_type={patch_type} (override; queue pop)")
        elif no_inspirations:
            # Sample coding type
            # Filter out any crossover variant if no inspirations are available
            valid_types = [
                t for t in self.patch_types if not t.startswith("cross")
            ]
            valid_probs = [
                p
                for t, p in zip(self.patch_types, self.patch_type_probs)
                if not t.startswith("cross")
            ]
        ...
        if (
            meta_recommendations not in [None, "none"]
            and not patch_type.startswith("cross")
        ):
        ...
        elif patch_type == "diff_ablate":
            sys_msg += DIFF_ABLATE_SYS_FORMAT
        ...
        elif patch_type.startswith("cross"):
        ...
        if patch_type == "diff":
            ...
            iter_msg = DIFF_ABLATE_ITER_MSG.format(
                language=self.language,
                code_content=parent.code,
                performance_metrics=perf_str(
                    parent.combined_score, parent.public_metrics
                ...
                text_feedback_section=text_feedback_section,
                previous_attempts=previous_attempts or "(no previous attempts recorded yet)",
            )
        elif patch_type == "full":
        ...
        elif patch_type == "cross_combine":
            iter_msg = CROSS_COMBINE_ITER_MSG.format(
                language=self.language,
                code_content=parent.code,
                performance_metrics=perf_str(
                    parent.combined_score, parent.public_metrics
        ...
        elif patch_type == "cross_translocate":
            iter_msg = CROSS_TRANSLOCATE_ITER_MSG.format(
                language=self.language,
                code_content=parent.code,
                performance_metrics=perf_str(
                    parent.combined_score, parent.public_metrics
