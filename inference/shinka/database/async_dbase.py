...
from .score import compute_effective_scores, wrap_rows
    ...
    async def compute_percentile_async(
        ...
        try:
            ...
            def compute_percentile_thread_safe():
                ...
                If raw is provided, the percentile is computed in the adaptive
                bucket-interpolated effective-score space (the candidate is
                merged into the population for ranking). Otherwise falls back
                to the legacy raw `combined_score` comparison.
                """
                thread_db = None
                try:
                    from .dbase import ProgramDatabase
                    ...
                    if raw is not None:
                        # Effective-score path: pull metric dicts and re-rank.
                        if correct_only:
                            thread_db.cursor.execute(
                                "SELECT id, combined_score, public_metrics, "
                        ...
                        wrapped = wrap_rows(rows)
                        candidate = SimpleNamespace(
                            id="__candidate__",
                            combined_score=score,
                            public_metrics=(
                                {"backward_score": bw} if bw is not None else {}
                            ),
                            private_metrics={"reported_sum_of_radii": raw},
                        )
                        all_progs = wrapped + [candidate]
                        eff_scores = compute_effective_scores(all_progs)
                        cand_eff = eff_scores[-1]
                        peer_effs = eff_scores[:-1]
                        beats = sum(1 for s in peer_effs if cand_eff > s)
                        ties = sum(1 for s in peer_effs if cand_eff == s)
                        return (beats + 0.5 * ties) / len(peer_effs)

                    # Get all scores from correct programs (legacy path)
                    if correct_only:
                        thread_db.cursor.execute(
