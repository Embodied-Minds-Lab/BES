...
class ProgramDatabase:
    ...
    def get_best_program(self, metric: Optional[str] = None) -> Optional[Program]:
        ...
        if metric:
            ...
            sorted_p = sort_by_effective(progs_with_cs, reverse=True)
            log_key = "combined_score (effective)"
        else:
    ...
    def get_top_programs(
        ...
        if metric == "combined_score":
            # Adaptive bucket interp depends on the full candidate set, so we
            # cannot push ORDER BY/LIMIT into SQL. Fetch all, re-rank below.
            base_query = """
        ...
        # combined_score path: re-rank by adaptive effective score (not by
        # raw combined_score) before truncating to n.
        if metric == "combined_score" and programs:
            return sort_by_effective(programs, reverse=True)[:n]
        if metric == "timestamp" and programs:
            return programs[:n]
    ...
    def _compute_archive_score_ranked(
        ...
        if not archive_programs:
            ...
            if primary_criterion == "combined_score":
                value = compute_effective_scores([program])[0]
            else:
        ...
        # Pre-compute effective scores once for the combined_score criterion.
        # This is set-aware: bw acts as intra-bucket sub-rank within all_programs.
        eff_scores_for_combined = compute_effective_scores(all_programs)

        score = 0.0
        for criterion, weight in criteria.items():
            # Get values for all programs
            if criterion == "combined_score":
                values = list(eff_scores_for_combined)
            else:
                values = [
                    self._get_criterion_value(p, criterion) for p in all_programs
                ]
    ...
    def _is_better(
        ...
        if use_ranked:
            ...
            # Simple single-criterion comparison: use adaptive effective score.
            # Bucket landscape = {p1, p2} ∪ archive_programs (if provided).
            cmp_set = [program1, program2]
            if archive_programs:
                cmp_set = cmp_set + [
                    p
                    for p in archive_programs
                    if p.id not in (program1.id, program2.id)
                ]
            eff = compute_effective_scores(cmp_set)
            s1, s2 = eff[0], eff[1]
    ...
    def get_top_programs_thread_safe(
        ...
        try:
            ...
            # Re-rank by adaptive effective score before truncating to n.
            return sort_by_effective(programs, reverse=True)[:n]
