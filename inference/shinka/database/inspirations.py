...
from .score import compute_effective_scores, sort_by_effective
...
class ArchiveInspirationSelector(ContextSelectorStrategy):
    ...
    def sample_context(self, parent: Any, n: int) -> List[Any]:
        ...
        if num_elites > 0 and len(inspirations) < n and parent_island_idx is not None:
            # Adaptive bucket interp depends on the candidate set, so we cannot
            # push ORDER BY/LIMIT into SQL — fetch all island elites, re-rank,
            # then pick top.
            self.cursor.execute(
                """
            ...
            elite_progs: List[Any] = []
            for row in self.cursor.fetchall():
                prog = self.get_program(row["id"])
                if prog:
                    elite_progs.append(prog)
            elite_progs = sort_by_effective(elite_progs, reverse=True)
            for prog in elite_progs:
                if len(inspirations) >= n:
                    break
                if prog.id not in insp_ids:
                    inspirations.append(prog)
                    insp_ids.add(prog.id)
...
class TopKInspirationSelector(ContextSelectorStrategy):
    ...
    def sample_context(
        ...
        # Sort by adaptive bucket-interpolated effective score across this set;
        # programs without combined_score fall back to averaged public metrics.
        progs_with_cs = [p for p in archive_programs if p.combined_score is not None]
        progs_without_cs = [
            p for p in archive_programs if p.combined_score is None
        ]
        sorted_with_cs = sort_by_effective(progs_with_cs, reverse=True)
        sorted_without_cs = sorted(
            progs_without_cs,
            key=lambda p: (
        ...
        sorted_programs = sorted_with_cs + sorted_without_cs
...
class InspirationContextBuilder:
    ...
    def _get_score(self, prog: Any) -> float:
        """Get the score for sorting (single-program fallback for logging only).

        Note: real ranking happens in build_context() via compute_effective_scores
        across the entire context set so adaptive bucket interp can apply.
        """
    ...
    def build_context(
        ...
        if self.sort_order == "ascending":
            # Least-to-most: worst programs first, best last (by adaptive
            # effective score across this context set, with public-metric
            # fallback for any program lacking combined_score).
            with_cs = [p for p in unique_inspirations if p.combined_score is not None]
            without_cs = [
                p for p in unique_inspirations if p.combined_score is None
            ]
            asc_with_cs = sort_by_effective(with_cs, reverse=False)
            asc_without_cs = sorted(without_cs, key=self._get_score, reverse=False)
            # Programs without combined_score have lowest priority → put first
            sorted_inspirations = asc_without_cs + asc_with_cs
