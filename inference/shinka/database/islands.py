...
from .score import compute_effective_scores, sort_by_effective, wrap_rows
...
class ElitistMigrationStrategy(IslandMigrationStrategy):
    ...
    def _select_migrants(
        ...
        if island_elitism:
            ...
            # Adaptive bucket interp depends on the candidate set, so fetch
            # all eligible programs and re-rank in Python.
            self.cursor.execute(
                """
                SELECT id, combined_score, public_metrics, private_metrics
                FROM programs
                WHERE island_idx = ? AND generation > 0 AND correct = 1
                """,
                (source_idx,),
            )
            wrapped = wrap_rows(self.cursor.fetchall())
            ranked = sort_by_effective(wrapped, reverse=True)
            elite_ids = [ranked[0].id] if ranked else []
...
class CombinedIslandManager:
    ...
    def get_best_program(self) -> Optional[Dict]:
        """Get the best program by adaptive bucket-interpolated effective score.
        ...
        scores = compute_effective_scores(wrapped)
        best_idx = max(range(len(scores)), key=lambda i: scores[i])
        return dict(rows[best_idx])
    ...
    def _get_correct_children(
        ...
        """Get correct children of a program, ordered by effective score.
        ...
        """
        # Adaptive bucket interp depends on the candidate set, so we fetch all
        # children and re-rank in Python before applying the limit.
        self.cursor.execute(
            """
            SELECT * FROM programs
