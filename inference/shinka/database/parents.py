    ...
    def sample_parent(self) -> Any:
        ...
        if archived_rows:
            ...
            if archived_programs:
                # Sort by adaptive bucket-interpolated effective score (best first)
                archived_programs = sort_by_effective(archived_programs, reverse=True)
                logger.info(
        ...
        if correct_rows:
            ...
            if correct_programs:
                # Sort by effective score so power-law indexing matches rank
                correct_programs = sort_by_effective(correct_programs, reverse=True)
                alpha = getattr(self.config, "exploitation_alpha", 1.0)
...
class WeightedSamplingStrategy(ParentSamplingStrategy):
    ...
    def sample_parent(self) -> Any:
        ...
        effective_scores = compute_effective_scores(eligible_programs)
        logger.info(
            f"Island {self.island_idx} => Effective range=["
            f"{min(effective_scores):.6f}, {max(effective_scores):.6f}]"
        )
        ...
        for i, p in enumerate(eligible_programs):
            # performance (alpha_i) from effective bucket-interpolated score
            alpha_i = effective_scores[i]
            # children count (n_i)
