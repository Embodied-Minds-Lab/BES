...
async def apply_patch_async(
    ...
    try:
        ...
        # diff_ablate was repurposed to an abandon-and-pivot full-rewrite (used by
        # the backward callback after expand-exhausted rebuild) — despite the
        # historical name it now outputs <CODE>, not <DIFF>, so it routes to the
        # full-patch parser. cross_combine and cross_translocate also full-rewrite.
        if patch_type in [
            "full",
            "diff_ablate",
            "cross",
