...
def get_client_llm(
    ...
    if provider == "anthropic":
        ...
        huit_oai = os.getenv("HUIT_OPENAI_API_KEY", "").strip()
        if huit_oai:
            client = openai.OpenAI(
                api_key=huit_oai,
                base_url="https://go.apis.huit.harvard.edu/ais-openai-direct/v1",
                default_headers={"api-key": huit_oai},
                timeout=TIMEOUT,
            )
        else:
        ...
        huit = os.getenv("HUIT_GEMINI_API_KEY", "").strip()
        if huit:
            client = genai.Client(
                api_key="dummy",
                http_options=genai.types.HttpOptions(
                    base_url="https://go.apis.huit.harvard.edu/ais-google-gemini",
                    headers={"api-key": huit},
                    timeout=_google_genai_timeout_ms(),
                ),
            )
        else:
            client = genai.Client(
