...
def _run_root(results_dir: str) -> Path:
    """Results_dir is <run_root>/gen_<N>/results/; two `.parent` hops get us home."""
...
def aggregate_circle_packing_metrics(
    ...
    bw_satisfaction: Dict[str, float] = {}  # node_id -> self_score in [0,1]
    if tree is not None and tree.get("children"):
        from shinka.bidirectional_search import verify_tree, recursive_score, flatten_satisfaction
        from bench_hooks import _eval_vars_from_arrays
        verified = verify_tree(tree, _eval_vars_from_arrays(centers, radii))
    ...
    metrics = {
        "combined_score": float(bw_score) if bw_score is not None else float(reported_sum),
