...
class SingleTurnAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        rspo_prefix_ids = kwargs.get("rspo_prefix_ids", None)
        if rspo_prefix_ids is not None:
            rspo_prefix_ids = list(rspo_prefix_ids)
            generation_prompt_ids = prompt_ids + rspo_prefix_ids
        else:
        ...
        # Build response_ids / mask / logprobs, accounting for any prefix.
        if rspo_prefix_ids is not None:
            prefix_len = len(rspo_prefix_ids)
            max_cont = self.response_length - prefix_len
            continuation = output.token_ids[:max_cont]
            response_ids = rspo_prefix_ids + continuation
            response_mask = [1] * len(response_ids)
            if output.log_probs:
                response_logprobs = [0.0] * prefix_len + list(output.log_probs[:max_cont])
            else:
        ...
        output: AgentLoopOutput = AgentLoopOutput(
            prompt_ids=prompt_ids,  # original prompt (no prefix)
