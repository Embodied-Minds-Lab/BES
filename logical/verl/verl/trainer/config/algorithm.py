...
class AlgoConfig(BaseConfig):
    ...
    # RSPO (Risk-Sensitive GRPO) settings.
    # rspo_beta: risk parameter for the entropic risk measure.
    #   β > 0: risk-seeking (soft-max, emphasizes max@k outcomes)
    #   β < 0: risk-averse (soft-min, emphasizes min@k outcomes)
    #   β → 0: arithmetic mean (recovers standard GRPO subtree average)
    rspo_beta: float = 1.0
    # rspo_n_init: M – number of independent seed trajectories per prompt.
    rspo_n_init: int = 1
    # rspo_n_expand: N – number of random branch points sampled per tree per round.
    rspo_n_expand: int = 1
    # rspo_expand_rounds: L – number of sequential expansion rounds.
    # Total rollouts per prompt G = M × (L × N + 1).
    # Rounds are sequential (each round can branch from previous rounds' results).
    rspo_expand_rounds: int = 1
