...
@register_adv_est("grpo_term")
def compute_grpo_term_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    if config is not None:
        term_t = getattr(config, "term_temperature", 2.0)
        if isinstance(term_t, (DictConfig,)):
            term_t = float(term_t)

    exp_t = math.exp(term_t)
    ...
    with torch.no_grad():
        ...
        # Compute TERM-weighted advantage per group
        for idx, group_indices in id2indices.items():
            group_scores = torch.stack([scores[i] for i in group_indices])
            n_group = len(group_indices)

            # Count correct (reward > 0) and wrong (reward = 0)
            is_correct = (group_scores > 0).float()
            n_correct = is_correct.sum().item()
            n_wrong = n_group - n_correct

            # TERM weights
            denom = exp_t * max(n_correct, 1) + n_wrong
            w_correct = exp_t / denom
            w_wrong = 1.0 / denom

            # Weighted mean as baseline
            weights = torch.where(is_correct > 0, w_correct, w_wrong)
            baseline = (weights * group_scores).sum()

            # Compute advantages with TERM weights
            for j, i in enumerate(group_indices):
                w = w_correct if is_correct[j] > 0 else w_wrong
                adv = (group_scores[j] - baseline) * w * n_group  # scale by n to match GRPO magnitude
                if norm_adv_by_std_in_grpo:
                    std = torch.std(group_scores) if n_group > 1 else torch.tensor(1.0)
                    adv = adv / (std + epsilon)
                scores[i] = adv
...
@register_adv_est("grpo_maxrl")
def compute_grpo_maxrl_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """GRPO with MaxRL loss normalization (1/K instead of 1/G per group)."""
    scores = token_level_rewards.sum(dim=-1)  # (bs,)
    ...
    with torch.no_grad():
        ...
        for idx, group_indices in id2indices.items():
            g = len(group_indices)
            group_rewards = torch.stack([token_level_rewards[i].sum() for i in group_indices])
            k = (group_rewards > 0).sum().item()
            if 0 < k < g:
                scale = g / k
                for i in group_indices:
                    scores[i] = scores[i] * scale
            # k=0 or k=g → advantage already 0, no scaling needed
...
def _entropic_risk_measure(rewards: torch.Tensor, beta: float) -> float:
    """Compute the entropic risk measure R̃_β(v) = (1/β) log( (1/n) Σ exp(β r_i) ).

    For β → 0, returns the arithmetic mean (standard GRPO baseline).
    For β > 0, soft-max (risk-seeking, emphasizing max@k outcomes).
    For β < 0, soft-min (risk-averse, emphasizing min@k outcomes).
    ...
    scaled = beta * rewards.float()
    max_val = scaled.max()
    log_mean_exp = max_val + torch.log(torch.exp(scaled - max_val).mean())
    return (log_mean_exp / beta).item()


@register_adv_est("rspo")
def compute_rspo_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    **kwargs,
    ...
    """RSPO (Risk-Sensitive GRPO): per-step advantages via response prefix tree.

    Algorithm
    ---------
    For each prompt group (shared uid) with G rollouts:
    1. Build a prefix trie from response token sequences.
    2. Each trie node v aggregates its subtree leaf rewards via the entropic
       risk measure: R̃_β(v) = (1/β) log( (1/|L(v)|) Σ_{i∈L(v)} exp(β r_i) ).
    3. All node values are group-normalized:
       Â(v) = ( R̃_β(v) − mean_{u∈V(T)} R̃_β(u) ) / ( std_{u∈V(T)} R̃_β(u) + ε ).
    4. Token (i, t) inherits advantage Â(v(i, t)) from its tree node.
    ...
    Args:
        ...
        index: (bs,) uid array for prompt-level grouping.
        epsilon: numerical stability constant.
        config: AlgoConfig; uses ``rspo_beta`` (default 1.0).
        batch: must contain ``"responses"`` tensor (bs, response_length).

    Returns:
        (advantages, returns) both of shape (bs, response_length).
    """
    beta = getattr(config, "rspo_beta", 1.0) if config is not None else 1.0

    assert batch is not None and "responses" in batch, (
        "RSPO requires response token IDs. Ensure batch['responses'] is available "
        "and passed to the advantage estimator."
    )
    responses = batch["responses"]  # (bs, resp_len) token IDs

    scores = token_level_rewards.sum(dim=-1)  # (bs,) terminal reward per trajectory
    bs, resp_len = response_mask.shape
    advantages = torch.zeros(bs, resp_len, device=scores.device, dtype=scores.dtype)
    ...
    with torch.no_grad():
        ...
        for uid, idx_list in id2indices.items():
            G = len(idx_list)
            if G == 1:
                continue  # single trajectory → advantage = 0

            gi = torch.tensor(idx_list, device=scores.device, dtype=torch.long)
            group_rewards = scores[gi]  # (G,)
            group_tokens = responses[gi].long()  # (G, resp_len)

            if group_rewards.std() < 1e-12:
                continue  # all rewards identical → advantage = 0

            # ----------------------------------------------------------
            # Phase 1: Build prefix tree, collect per-node R̃_β values
            # ----------------------------------------------------------
            node_values: list[float] = []
            # node_idx_at[g, t] → index into node_values for trajectory g at position t
            node_idx_at = torch.full((G, resp_len), -1, dtype=torch.long)

            # Root node (depth 0, before any response token): all G trajectories
            node_values.append(_entropic_risk_measure(group_rewards, beta))

            # Incremental group-splitting: at each position t, combine the
            # current group id with the token to split groups that diverge.
            # After torch.unique the group ids are consecutive ints in [0, n).
            group_id = torch.zeros(G, dtype=torch.long, device=group_tokens.device)
            singleton_start = resp_len  # position where all groups become singletons

            for t in range(resp_len):
                tok = group_tokens[:, t]
                max_tok = tok.max().item() + 1
                composite = group_id * max_tok + tok
                _, group_id = torch.unique(composite, return_inverse=True)

                num_groups = group_id.max().item() + 1
                base_idx = len(node_values)

                # Compute R̃_β per group at this depth
                for gid in range(num_groups):
                    members = (group_id == gid).nonzero(as_tuple=False).view(-1)
                    node_val = _entropic_risk_measure(group_rewards[members], beta)
                    node_values.append(node_val)

                # Map each trajectory to its node at this depth
                node_idx_at[:, t] = group_id + base_idx

                # Early exit: once every trajectory is in its own group the
                # remaining positions all have R̃_β = r_i (singleton nodes).
                if num_groups == G:
                    singleton_start = t + 1
                    break

            # ----------------------------------------------------------
            # Phase 2: Compute normalisation statistics over ALL tree nodes
            # ----------------------------------------------------------
            existing_vals = torch.tensor(
                node_values, device=scores.device, dtype=scores.dtype
            )
            n_existing = existing_vals.numel()

            remaining = resp_len - singleton_start
            if remaining > 0:
                # Implicit singleton nodes: each trajectory g contributes r_g
                # repeated `remaining` times.
                n_singleton = G * remaining
                total_n = n_existing + n_singleton
                total_sum = existing_vals.sum() + group_rewards.sum() * remaining
                total_sum_sq = (existing_vals**2).sum() + (group_rewards**2).sum() * remaining
            else:
                total_n = n_existing
                total_sum = existing_vals.sum()
                total_sum_sq = (existing_vals**2).sum()

            mean_v = total_sum / total_n
            var_v = (total_sum_sq / total_n - mean_v**2).clamp(min=0)
            std_v = var_v.sqrt()

            # ----------------------------------------------------------
            # Phase 3: Assign normalised advantages
            # ----------------------------------------------------------
            # Non-singleton positions (from the explicit tree nodes)
            for g in range(G):
                valid = node_idx_at[g] >= 0
                if valid.any():
                    nidx = node_idx_at[g, valid]
                    advantages[gi[g], valid] = (existing_vals[nidx] - mean_v) / (std_v + epsilon)

            # Singleton positions (after full divergence)
            if remaining > 0:
                for g in range(G):
                    adv_g = (group_rewards[g] - mean_v) / (std_v + epsilon)
                    advantages[gi[g], singleton_start:resp_len] = adv_g
