    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.should_save_hf_model:
            ...
            if self.rank == 0:
                ...
                _mt = getattr(model_config, "model_type", "")
                if _mt == "gemma3" and hasattr(model_config, "text_config"):
                    from transformers import AutoModelForCausalLM, Gemma3TextConfig
                    auto_model_cls = AutoModelForCausalLM
                    save_config = Gemma3TextConfig(**model_config.text_config.to_dict())
                elif _mt == "gemma3_text":
                    from transformers import AutoModelForCausalLM
                    auto_model_cls = AutoModelForCausalLM
                elif "ForTokenClassification" in (getattr(model_config, "architectures", None) or [""])[0]:
                    from transformers import AutoModelForTokenClassification

                    auto_model_cls = AutoModelForTokenClassification
                elif "ForCausalLM" in (getattr(model_config, "architectures", None) or [""])[0]:
                    from transformers import AutoModelForCausalLM

                    auto_model_cls = AutoModelForCausalLM
                elif "ForConditionalGeneration" in (getattr(model_config, "architectures", None) or [""])[0]:
                    auto_model_cls = get_auto_model_for_vision2seq()
                else:
                    raise NotImplementedError(f"Unknown architecture {getattr(model_config, 'architectures', None)}")

                with init_empty_weights():
                    save_model = auto_model_cls.from_config(
                        save_config, torch_dtype=torch.bfloat16, trust_remote_code=self.trust_remote_code
                    )
