...
def hf_tokenizer(name_or_path, correct_pad_token=True, correct_gemma2=True, **kwargs):
    ...
    if tokenizer.chat_template and "Conversation roles must alternate" in tokenizer.chat_template:
        import re
        tokenizer.chat_template = re.sub(
            r'\{%-?\s*if\s+\(message\[.role.\]\s*==\s*.user.\)\s*!=\s*\(loop\.index0\s*%\s*2\s*==\s*0\).*?endif\s*-?%\}',
            '',
