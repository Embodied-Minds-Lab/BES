...
class FSDPEngine(BaseEngine):
    ...
    def _build_module(self):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if self.model_config.model_type == "language_model":
                # Gemma3 special handling: composite architecture (text+vision)
                # causes _no_split_modules mismatch. Load text-only config/model.
                hf_config = self.model_config.hf_config
                if getattr(hf_config, "model_type", "") in ("gemma3", "gemma3_text"):
                    from transformers import Gemma3ForCausalLM, Gemma3TextConfig
                    if hasattr(hf_config, "text_config"):
                        text_config = Gemma3TextConfig(**hf_config.text_config.to_dict())
                    else:
                        text_config = hf_config
                    module = Gemma3ForCausalLM.from_pretrained(
                        pretrained_model_name_or_path=self.model_config.local_path,
                        torch_dtype=torch_dtype,
                        config=text_config,
                        trust_remote_code=self.model_config.trust_remote_code,
                    )
                    # Override _no_split_modules to only include text decoder layer
                    module._no_split_modules = ["Gemma3DecoderLayer"]
                else:
    ...
    def get_per_tensor_param(self, layered_summon=False, base_sync_done=False, **kwargs):
        ...
        # Gemma3 composite model: FSDP loads text-only Gemma3ForCausalLM but
        # vLLM loads the full Gemma3ForConditionalGeneration which wraps the
        # text model under "language_model.". Add the prefix so weight sync matches.
        hf_config = self.model_config.hf_config
        if getattr(hf_config, "model_type", "") == "gemma3" and hasattr(hf_config, "text_config"):
            params = {f"language_model.{k}": v for k, v in params.items()}
