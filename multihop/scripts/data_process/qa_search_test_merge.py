...
if __name__ == '__main__':
    ...
    parser.add_argument('--flashrag_dir', default='/path/to/FlashRAG_Dataset')
    ...
    for data_source in data_sources:
        ...
        src_dir = os.path.join(args.flashrag_dir, data_source)
        test_jsonl = os.path.join(src_dir, 'test.jsonl')
        dev_jsonl = os.path.join(src_dir, 'dev.jsonl')
        train_jsonl = os.path.join(src_dir, 'train.jsonl')

        if os.path.isdir(src_dir):
            if os.path.isfile(test_jsonl):
                print(f'Using the {data_source} test dataset...')
                chosen = test_jsonl
            elif os.path.isfile(dev_jsonl):
                print(f'Using the {data_source} dev dataset...')
                chosen = dev_jsonl
            else:
                print(f'Using the {data_source} train dataset...')
                chosen = train_jsonl
            test_dataset = datasets.load_dataset('json', data_files=chosen, split='train')
        else:
            if data_source != 'strategyqa':
                dataset = datasets.load_dataset(args.flashrag_dir, data_source)
            else:
