...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, risk_beta=1.0):
    ...
    if adv_estimator == 'gae':
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_grpo_maxrl_outcome_advantage(
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_grpo_outcome_advantage_term(token_level_rewards=token_level_rewards,
                                                                             eos_mask=response_mask,
                                                                             index=index,
                                                                             beta=risk_beta)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == 'tree_term':
        token_level_rewards = data.batch['token_level_rewards']
        index = data.non_tensor_batch['uid']
        tree_index = data.non_tensor_batch['tree_uid']
        responses = data.batch['responses']
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        inter_tree_advantages, inter_tree_returns = core_algos.compute_grpo_outcome_advantage_term(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index,
                                                                        beta=risk_beta)
        inner_tree_advantages, inner_tree_returns = core_algos.compute_grpo_outcome_advantage_term(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=tree_index,
                                                                        beta=risk_beta)
        ...
        data.batch['advantages'] = inter_tree_advantages + inner_tree_advantages
        data.batch['returns'] = inter_tree_returns + inner_tree_returns
    elif adv_estimator == 'tree_inner_term':
        token_level_rewards = data.batch['token_level_rewards']
        tree_index = data.non_tensor_batch['tree_uid']
        responses = data.batch['responses']
        response_length = responses.size(-1)
        ...
        data.batch['advantages'] = inner_tree_advantages
        data.batch['returns'] = inner_tree_returns
    elif adv_estimator == 'tree_2norm_term':
        token_level_rewards = data.batch['token_level_rewards']
        index = data.non_tensor_batch['uid']
        tree_index = data.non_tensor_batch['tree_uid']
        responses = data.batch['responses']
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        tree_advantages, tree_returns = core_algos.compute_tree_grpo_outcome_advantage_term(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index,
                                                                        tree_index=tree_index,
                                                                        beta=risk_beta)
...
def compute_data_metrics(batch, use_critic=True, reward_mode='tree'):
    # token_level_{scores,rewards} are (batch, seq_len=max_response_length).
    # Use sum(-1) for per-trajectory totals (this aggregation only affects
    # logging, not the loss).
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        # Prefer StatefulDataLoader for resume support; fall back to plain DataLoader if torchdata missing.
        try:
        ...
        self.train_dataloader = _DataLoader(dataset=self.train_dataset,
        ...
        self.val_dataloader = _DataLoader(dataset=self.val_dataset,
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            tool_call_mode = self.config.actor_rollout_ref.rollout.get('tool_call_mode', False),
            tool_call_stub_response = self.config.actor_rollout_ref.rollout.get('tool_call_stub_response', 'ok'),
        )
    ...
    def init_workers(self):
        ...
        if self.config.algorithm.adv_estimator == 'gae':
            ...
            self.use_critic = True  
                                                     ...
                                                     'grpo_term', 'tree_term', 'tree_inner_term', 'tree_2norm_term',
                                                     'grpo_maxrl']:
    ...
    def _save_checkpoint(self):
        # If trainer.remove_previous_ckpt=true, FSDP checkpoint manager rmtree's the
        # previous global_step_N dir on disk so only the latest checkpoint is kept.
        keep_only_latest = bool(self.config.trainer.get('remove_previous_ckpt', False))
        ...
        self.actor_rollout_wg.save_checkpoint(actor_local_path, actor_remote_path,
                                              ...
                                              remove_previous_ckpt=keep_only_latest)
        ...
        # Save dataloader iterator state alongside actor shards (only if it's a StatefulDataLoader).
        if hasattr(self.train_dataloader, 'state_dict'):
            os.makedirs(actor_local_path, exist_ok=True)
            data_path = os.path.join(actor_local_path, 'data.pt')
            torch.save(self.train_dataloader.state_dict(), data_path)
            print(f'[ckpt] saved dataloader state to {data_path}')

        # Atomic latest-step pointer for `auto`-mode resume in the future.
    ...
    def _maybe_load_checkpoint(self):
        """If trainer.resume_from_path is set, load FSDP shards and set self.global_steps accordingly.

        Expected resume_from_path: '/.../checkpoints/<exp>/actor/global_step_N' (the actor dir).
        Critic, if used, is loaded from the parallel '/.../<exp>/critic/global_step_N' path.
        ...
        if not resume_path:
            return False
        assert os.path.isdir(resume_path), f'resume_from_path does not exist: {resume_path}'
        print(f'[resume] loading actor checkpoint from {resume_path}')
        self.actor_rollout_wg.load_checkpoint(resume_path)
        if self.use_critic:
            critic_path = resume_path.replace('/actor/', '/critic/')
            assert os.path.isdir(critic_path), f'critic resume path missing: {critic_path}'
            print(f'[resume] loading critic checkpoint from {critic_path}')
            self.critic_wg.load_checkpoint(critic_path)
        m = re.search(r'global_step_(\d+)', resume_path)
        if m:
            self.global_steps = int(m.group(1))
        else:
            self.global_steps = int(self.config.trainer.get('resume_from_step', 0))
        print(f'[resume] global_steps set to {self.global_steps}')

        # Restore dataloader iterator position so we resume from the same batch (not epoch start).
        data_path = os.path.join(resume_path, 'data.pt')
        if os.path.exists(data_path) and hasattr(self.train_dataloader, 'load_state_dict'):
            steps_per_epoch = len(self.train_dataloader)
            at_epoch_boundary = steps_per_epoch > 0 and self.global_steps % steps_per_epoch == 0
            if at_epoch_boundary:
                print(f'[resume] dataloader at epoch boundary (step {self.global_steps} / {steps_per_epoch}); '
                      f'will start fresh epoch from scratch')
            else:
                state = torch.load(data_path, weights_only=False)
                self.train_dataloader.load_state_dict(state)
                print(f'[resume] loaded dataloader state from {data_path}')
        elif not hasattr(self.train_dataloader, 'load_state_dict'):
            print('[resume] WARNING: dataloader is plain DataLoader (no state_dict); install torchdata for full resume')
        else:
            print(f'[resume] WARNING: no data.pt at {data_path}; dataloader will start from epoch 0')
        return True
    ...
    def fit(self):
        ...
        logger = self.logger
        resumed = self._maybe_load_checkpoint()
        if not resumed:
            self.global_steps = 0
        # perform validation before training
        # currently, we only support validation using the reward_function.
        # Skip pre-training val on resume — it was already logged in the prior run.
        if not resumed and self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
        ...
        # we start from step N+1 (where N is the resumed step or 0)
        self.global_steps += 1

        use_bes = bool(self.config.actor_rollout_ref.rollout.get('use_bes', False))

        if use_bes:
            from search_r1.llm_agent.generation_bes import (
                GenerationBidirectionalConfig,
                LLMGenerationBidirectionalAsyncManager,
            )
            bes_cfg_node = self.config.actor_rollout_ref.rollout.get('bes', {})
            gen_config = GenerationBidirectionalConfig(
                max_turns=self.config.max_turns,
                max_start_length=self.config.data.max_start_length,
                max_prompt_length=self.config.data.max_prompt_length,
                max_response_length=self.config.data.max_response_length,
                max_obs_length=self.config.data.max_obs_length,
                num_gpus=self.config.trainer.n_gpus_per_node * self.config.trainer.nnodes,
                budget=bes_cfg_node.get('budget', 30),
                sim_threshold=bes_cfg_node.get('sim_threshold', 0.6),
                no_span_abstract=bes_cfg_node.get('no_span_abstract', False),
                search_url=self.config.retriever.url,
                topk=self.config.retriever.topk,
                backward_url=bes_cfg_node.get('backward_url', 'http://localhost:8236/v1'),
                backward_model=bes_cfg_node.get('backward_model',
                                                'meta-llama/Llama-3.1-8B-Instruct'),
                embedder_name=bes_cfg_node.get('embedder',
                                              'sentence-transformers/all-MiniLM-L6-v2'),
                embedder_device=bes_cfg_node.get('embedder_device', 'cuda'),
                grpo_n=bes_cfg_node.get('grpo_n', 8),
                think_prefix=bes_cfg_node.get('think_prefix', False),
                k_parallel=bes_cfg_node.get('k_parallel', 1),
            )
            print('[ray_trainer_ts] using BES (async cross-Q driver)', flush=True)
            generation_manager = LLMGenerationBidirectionalAsyncManager(
                tokenizer=self.tokenizer,
                actor_rollout_wg=self.actor_rollout_wg,
                config=gen_config,
            )
                ...
                # with _timer('step', timing_raw):
                    if True:
                        ...
                        with _timer('gen', timing_raw):
                            generation_manager.timing_raw = timing_raw
                            if use_bes:
                                final_gen_batch_output = generation_manager.run_llm_loop_bidirectional(
                                    gen_batch=gen_batch,
                                    initial_input_ids=first_input_ids,
                                )
                            else:
                        ...
                        # repeat to align with repeated responses in rollout
                        if use_bes:
                            grpo_n = self.config.actor_rollout_ref.rollout.get('bes', {}).get('grpo_n', 8)
                            batch = batch.repeat(repeat_times=grpo_n, interleave=True)
                        else:
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        batch = compute_advantage(batch,
                                                  ...
                                                  risk_beta=self.config.algorithm.get('risk_sensitive_beta', 1.0),
                                                  )
