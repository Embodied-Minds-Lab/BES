...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        skip_cycle = os.environ.get('VERL_SKIP_SLEEP_WAKE', '0') == '1'
        if skip_cycle and getattr(self, '_weights_synced', False):
            # val-only mode: weights don't change across turns; skip re-sync and wake
            if self.device_mesh is not None:
                self.torch_random_states = torch.cuda.get_rng_state()
        ...
        if vllm_version in ('0.4.2', '0.5.4', '0.6.3'):
            ...
            if load_format == 'dtensor':
                ...
                with set_default_torch_dtype(next(vllm_model.parameters()).dtype):
                    if getattr(vllm_model.config, 'tie_word_embeddings', False) and 'lm_head.weight' in params:
                        params.pop('lm_head.weight', None)
                    vllm_model.load_weights(params.items())
